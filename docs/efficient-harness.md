...
# 目标五：分析成熟加速方法的 insight 与 method

单纯把 trace、profiling、干预接口做好还不够——这套观测能力存在的意义之一，是能拿它去检验学界/工业界已经提出的 agent 加速方法：它们声称要解决的问题是否真的出现在我们自己的 workload 里，它们的核心方法能不能用目标三的标准化干预接口实现出来，效果能不能用目标二的三层 profiling 数据验证。

对每一篇纳入分析的论文，要拆出两层东西，不是简单摘要：

- **Insight（为什么有效）**：论文观察到的、支撑其方法成立的底层事实或假设（比如"tool 执行的墙钟时间里 GPU 是空闲的""相邻请求的 prompt 前缀高度重复""不是所有 token 对下游任务同等重要"）。
- **Method（怎么落地）**：把 insight 转成的具体机制（推测执行、前缀复用调度、KV cache 淘汰策略、prompt 压缩、模型级联……）。

同时要用我们自己已有的真实数据检验：论文的 insight 是否在我们已跑过的 benchmark/workload 里成立（成立、不成立、还是测的是不同指标——量出来之前不能假设一致）；论文的 method 落在目标一到目标四的哪一层/哪个干预点；如果要在我们的 harness 上实现或验证，还缺什么（可能缺一个真实会有网络/执行延迟的 workload，而不是缺代码）。

这不是一次性任务，是要持续积累的清单，见 [`docs/acceleration_methods_survey.md`](./acceleration_methods_survey.md)。
